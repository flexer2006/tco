...
var (
	ErrAuthBackendNil       = errors.New("telegram auth backend is nil")
	ErrOnboardingServiceNil = errors.New("onboarding service must not be nil")
	ErrSessionPathEmpty     = errors.New("session path must not be empty")
...
func (e invalidTransitionError) Error() string {
	...
	return fmt.Sprintf(
		"%s: transition from %q is not allowed (allowed: %s)",
		e.Action,
		e.From,
