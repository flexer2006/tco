...
func wireToManifest(value manifestWire) (domain.Manifest, error) {
	if value.SchemaVersion != domain.SchemaVersion {
		return domain.Manifest{}, fmt.Errorf(
			"%w %d",
...
func noteFromWire(record noteWire) (domain.NoteRecord, error) {
	...
	noteRecord, err := domain.NewNoteRecord(
		...
		domain.NoteID(record.DuplicateOf),
	)
	if err != nil {
		return domain.NoteRecord{}, err
	}
	...
	if noteRecord.Hash() != record.Hash {
		return domain.NoteRecord{}, ErrHashMismatch
	}
...
func clusterFromWire(record clusterWire) (domain.ClusterRecord, error) {
	...
	return domain.NewClusterRecord(
		record.ID,
		record.Name,
		record.Slug,
		centroid,
...
func runMetadataFromWire(record runMetadataWire) (domain.RunMetadata, error) {
	thresholds, err := domain.NewThresholds(
		record.Thresholds.DedupSimilarity,
		record.Thresholds.ClusterSimilarity,
	)
	if err != nil {
	...
	counts, err := domain.NewCounts(
		record.Counts.Notes,
		record.Counts.CanonicalNotes,
		record.Counts.DuplicateNotes,
		record.Counts.Clusters,
	)
	if err != nil {
		return domain.RunMetadata{}, err
	}

	timestamps, err := domain.NewTimestamps(
		record.Timestamps.StartedAtUTC.UTC(),
		record.Timestamps.FinishedAtUTC.UTC(),
	)
	if err != nil {
		return domain.RunMetadata{}, err
	}

	return domain.NewRunMetadata(
		record.RunID,
		record.RunMode,
		record.BatchMode,
		record.BatchSize,
