...
func authPageCSS() string {
	return authPageCSSLayout() + authPageCSSControls()
}

func authPageCSSLayout() string {
	return `
    :root {
      --bg: #f2f6fb;
      --card: #ffffff;
...
func authPageCSSControls() string {
	return `
    .grid {
      display: grid;
...
func authPageBody(state, phone, escapedCSRFToken string) string {
  ...
  <main class="panel">
    <header class="head">
      ...
      <p class="subtitle">
        Complete steps from left to right: start login, enter code from Telegram,
        and enter cloud password only if Telegram asks for 2FA.
      </p>
      <div class="meta">
