...
func isAuthError(err error) bool {
	...
	return tgauth.IsUnauthorized(err) || tgerr.IsCode(err, telegramRPCUnauthorized)
}
...
func isRateLimitError(err error) bool {
	...
	return tgerr.IsCode(err, telegramRPCFloodWait)
}
...
func adviceForLiveErrorKind(kind liveErrorKind) string {
	...
	case liveErrorKindAuth:
		return "verify TELEGRAM_API_ID/TELEGRAM_API_HASH and ensure a valid authorized session exists at " +
